...
function RewindHandler:update(dt, player)
    if self.is_rewinding then
        ...
        if self.rewind_timer <= 0 then
            ...
            -- Snap to the final position and animation state
            ...
            if self.rewind_path[index1] and self.rewind_path[index2] then
                ...
                -- Set animation to the current point in history (no interpolation for frames)
        ...
        -- Record history when not rewinding
...
function RewindHandler:start_rewind(player)
    ...
    -- The first point in our path is the player's current state.
    ...
    -- Go through history backwards and add points to the path
